...
class EditResendService extends TypertRemoteService {
	...
	/**
	 * List archived sessions with identifying metadata (for the settings-page
	 * restore panel). The official API exposes no archive listing surface
	 * beyond the id set, so titles fall back to cwd/createdAt from the session
	 * corpus.
	 * @param request - ignored (`{}`).
	 * @returns `{ ok: true, items: [{ sessionId, cwd, createdAt }] }` or an error.
	 */
	async listArchived() {
		...
		if (query !== undefined && typeof query.listSessions === "function") {
			try {
				for (const rec of await query.listSessions()) byId.set(rec.header.id, rec.header);
			} catch (e) { /* keep empty */ }
		}
		const items = archived.map((id) => {
			...
			return {
				...
				createdAt: header && Number.isSafeInteger(header.createdAt) ? header.createdAt : null
			};
		});
	...
	/**
	 * Unarchive a session (restore it to its grouping surfaces). The official
	 * workspace API has no unarchive method, so this updates the registry
	 * state directly; the domain change broadcast (`host/archived-sessions-
	 * changed`) refreshes the client UI automatically.
	 * @param request - `{ sessionId }`.
	 * @returns `{ ok: true }` or `{ ok: false, error }`.
	 */
	/**
	 * Archive a session (hide it from grouping/search). Kept OUT of `edit`:
	 * archiving the currently-open session inside the edit RPC lets the
	 * `host/archived-sessions-changed` broadcast race the client's navigation
	 * to the fork (the client clears a current session that became archived,
	 * landing the user on the new-session page). The client instead opens the
	 * fork FIRST, then calls this — by then the current session is the fork,
	 * so the clearing rule can never fire.
	 * @param request - `{ sessionId }`.
	 * @returns `{ ok: true }` or `{ ok: false, error }`.
	 */
	async archive(request) {
		...
		if (typeof sessionId !== "string" || sessionId === "") return { ok: false, error: "invalid-args" };
		const registry = this.ctx.workspaceRegistry;
		if (registry === undefined) return { ok: false, error: "workspace-registry-unavailable" };
		try {
			...
			return { ok: false, error: "archive-failed", message: String(e) };
		}
	}
	...
	async unarchive(request) {
		...
		try {
			const archived = [...registry.archivedSessionIds];
			if (!archived.includes(sessionId)) return { ok: true }; // idempotent
			const workspaceIds = registry.list().map((w) => w.id);
			...
			return { ok: false, error: "unarchive-failed", message: String(e) };
		}
	}
	...
	async edit(request) {
		...
		// 9. NO archive here. The source is archived by the client AFTER it
		//    opens the fork (see `archive`): archiving the currently-open
		//    session from inside this RPC would broadcast archived-sessions-
		//    changed while the client still has the source current, and the
		//    client's projection clears a current session that became archived
		//    — the user would be thrown to the new-session page instead of the
		//    fork. The client's ordering (open fork, then archive) closes that
		//    race: by archive time the current session is already the fork.
...
for (const [methodName, endpointName] of [["edit", "edit"], ["getText", "getText"], ["unarchive", "unarchive"], ["listArchived", "listArchived"], ["archive", "archive"]]) {
